...
from flask_restful import (Resource, Api, reqparse, inputs,
                           ...
                           url_for, abort)
import models
# all the fileds to include when api is called
course_fields = {
    'id': fields.Integer,
    'title': fields.String,
    'url': fields.String,
...
def add_reviews(course):
    course.reviews = [url_for(
        'resources.reviews.review', id=review.id
...
class CourseList(Resource):
    ...
    def post(self):
        ...
        return (add_reviews(course), 201,
                {'Location': url_for(
            'resources.courses.course', id=course.id)})
...
class Course(Resource):
    ...
    @marshal_with(course_fields)# when we return dingle object
    def get(self, id):
    ...
    def put(self, id):
        ...
        return (add_reviews(models.Course.get(models.Course.id==id)),
                200, {'Location': url_for('resources.courses.course', id=id)})

    def delete(self, id):
        query = models.Course.delete().where(models.Course.id==id)
        query.execute()
        return '', 204, {'Location': url_for('resources.courses.course', id=id)}


courses_api = Blueprint('resources.courses', __name__)
api = Api(courses_api)
api.add_resource(CourseList, '/api/v1/courses',
                 endpoint = 'courses')
api.add_resource(Course, '/api/v1/courses/<int:id>',
                 endpoint = 'course')